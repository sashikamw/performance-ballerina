...
def save_lmplots(df_all):
    ...
    for ycolumn, ylabel, ychart in zip(ycolumns, ylabels, ycharts):
        for xcolumn, xchart in zip(xcolumns, xcharts):
            ...
            ballerinachart.save_lmplot(df_all, chart, xcolumn, ycolumn, title, ylabel=ylabel)
            ballerinachart.save_lmplot(df_all, chart + '_with_hue', xcolumn, ycolumn, title, hue='Name', ylabel=ylabel)
...
def save_point_plots(df_all):
    ...
    for sleep_time in unique_sleep_times_in_df_all:
        for message_size in unique_message_sizes_in_df_all:
            ...
            chart_suffix = '_' + ballerinachart.format_time(sleep_time) + '_' + ballerinachart.format_bytes(
                message_size)
            ...
            for ycolumn, ylabel, chart in zip(ycolumns, ylabels, charts):
                ballerinachart.save_point_plot(df_filtered, chart + chart_suffix, 'Concurrent Users', ycolumn,
...
def save_multi_columns_categorical_charts(df, chart, sleep_time, columns, y, hue, title, kind='point'):
    ...
    for column in columns:
        ...
        ballerinachart.save_multi_columns_categorical_charts(df.loc[df['Sleep Time (ms)'] == sleep_time],
...
def save_bar_plot(df, chart, sleep_time, message_size, columns, y, hue, title):
    ...
    ballerinachart.save_bar_plot(df_results, chart, 'Concurrent Users', y, title, hue=hue)
...
def save_comparison_plots(df):
    ...
    for sleep_time in unique_sleep_times_in_df:
        ...
        for message_size in unique_message_sizes_in_df:
            ...
            title_suffix = " for " + message_size + " messages with " + ballerinachart.format_time(
...
def save_single_comparison_plots_by_sleep_time(df, chart, unique_message_sizes, columns, y, hue, title, kind='point'):
    ...
    for column in columns:
        ...
        ballerinachart.save_multi_columns_categorical_charts(df, chart, comparison_columns, y, hue, title,
                                                             len(columns) == 1,
